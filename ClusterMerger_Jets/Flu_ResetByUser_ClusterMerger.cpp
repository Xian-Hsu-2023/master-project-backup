...
// added by XinaHsu
extern int    JetBModel;               // (0 for O'Neill's toroidal model)
extern double P_inj_Const, T_inj, f_B; // jets power; injection time (in single injection); B energy fraction
extern double JetBModel1_AFactor;      // when jets B field model==1, assume constant current inside a = A * Rj
...
//    Transfer into BH frame
      // stop transfering into BH frame for jets injection (Xian)
...
#  endif
   ...
   // reset MHD variables first (xianhsu, almost directly copy from Fluid/Flu_ResetByUser.cpp)
#  ifdef MHD
   if ( ResetMag ) {
...
// Get the BH position and velocity and adjust them if needed
   bool AdjustPosNow = false;  // try not to adjust in injection simulation
...
// calculate the injection rate
   for (int c=0; c<Merger_Coll_NumHalos; c++){
      ...
      // added by XianHsu (only for single cluster and single injection)
      if (Single_Injection){
         if (TimeNew <= T_inj){
            Edot[c] = P_inj_Const;
            Edot[c] *= (1-f_B);
            // Aux_Message( stdout, "  Injection power = %g\n", Edot[c]);
            // Pdot[c] = sqrt(2) * Edot[c] / (Const_c/UNIT_V);
            Pdot[c] = sqrt(2*eta/eps_f*(1.0-eps_m)) * Edot[c] / (Const_c/UNIT_V);
            // Aux_Message( stdout, "  Injection momentum = %g\n", Pdot[c]);
            // Mdot[c] = pow(Pdot[c], 2) / (2*Edot[c]);
            Mdot[c] = eta*Edot[c]/eps_f/pow(Const_c/UNIT_V, 2);
         }
         else{
            Mdot[c] = 0.0;
            Pdot[c] = 0.0;
...
*/
   if (!(Single_Injection && TimeNew > T_inj)){
...
double MHD_ResetByUser_VecPot_ClusterMerger( const double x, const double y, const double z, const double Time,
                                         const double dt, const int lv, const char Component, double AuxArray[] )
{
   double A;
   // Not available yet
...
} // FUNCTION : MHD_ResetByUser_VecPot_ClusterMerger

double MHD_ResetByUser_BField_ClusterMerger( const double x, const double y, const double z, const double Time,
                                             const double dt, const int lv, const char Component, double AuxArray[], const double B_in,
                                             const bool UseVecPot, const real *Ax, const real *Ay, const real *Az,
                                             const int i, const int j, const int k )
{
   const double r0   = Jet_Radius1;
   const double h0   = Jet_HalfHeight1;
   ...
   */
   ...
   EB = P_inj_Const * f_B;
   // B0 = 0.0;
   // B0 = sqrt(8.0*EB / POW(r0, 3));
   // B0 = sqrt(2*EB / POW(Jet_Radius1, 3) / M_PI);
   // Aux_Message( stdout, "  B energy               = %g\n",           EB );
   // Aux_Message( stdout, "  B energy coefficient   = %g\n",           B0 );

   if (!( Single_Injection && Time > T_inj ) && ( abs(dz) <= h0 && r <= r0 * JetBModel2_BFactor ))
   {
      if (JetBModel == 0)
      {
         // Aux_Message( stdout, "  toroidal injection/n");
         switch ( Component )
         {
            B0 = sqrt(8.0*EB / POW(r0, 3));
            // case 'x' :  B_out = B_in - dt * B0 * dy / r0;  if (Time != 0.0) Aux_Message( stdout, "  dt = %g  EB = %g  r0 = %g  B0 = %g  dx = %g  dy = %g  dz = %g  r = %g\n", dt, EB, r0, B0, dx, dy, dz, r);break;
            // case 'x' :  B_out = dt * B0 * dy / r / r0;  break;
            // case 'y' :  B_out = dt * B0 * dx / r / r0;  break;
            // case 'z' :  B_out = 0.0;                    break;
            // case 'x' :  B_out = dt * B0 * dy / r0;  break;
            // case 'y' :  B_out = dt * B0 * dx / r0;  break;
            // case 'z' :  B_out = 0.0;                break;
            case 'x' :  B_out = B_in - dt * B0 * dy / r0;  break;
            case 'y' :  B_out = B_in + dt * B0 * dx / r0;  break;
      ...
      else if (JetBModel == 1)
      {
         ...
         if (r > (JetBModel1_AFactor*r0))
         {
            switch ( Component )
            {
               case 'x' :  B_out = B_in - dt * B0 * dy * (JetBModel1_AFactor*r0) / POW(r, 2);  break;
               case 'y' :  B_out = B_in + dt * B0 * dx * (JetBModel1_AFactor*r0) / POW(r, 2);  break;
               case 'z' :  B_out = B_in;                                  break;
               
               default  :  Aux_Error( ERROR_INFO, "unsupported component (%c) !!\n", Component );
            }
         }
         else if (r <= (JetBModel1_AFactor*r0))
         {
            switch ( Component )
            {
               case 'x' :  B_out = B_in - dt * B0 * dy / (JetBModel1_AFactor*r0);  break;
               case 'y' :  B_out = B_in + dt * B0 * dx / (JetBModel1_AFactor*r0);  break;
               case 'z' :  B_out = B_in;                      break;
      ...
      {
         B0 = sqrt(EB/((r0/2) * POW(r0, 2) * (0.25+log(JetBModel2_BFactor))));
         if (r > (r0))
         {
            switch ( Component )
            {
               case 'x' :  B_out = B_in - dt * B0 * dy * r0 / POW(r, 2);  break;
               case 'y' :  B_out = B_in + dt * B0 * dx * r0 / POW(r, 2);  break;
               case 'z' :  B_out = B_in;                                  break;
      ...
      {
         Aux_Error( ERROR_INFO, "unsupported magnetic field morphology !!\n" );
      }
   }
   else
   {
