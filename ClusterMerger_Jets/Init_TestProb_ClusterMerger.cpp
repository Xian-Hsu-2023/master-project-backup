...
static FieldIdx_t ColorField3Idx = Idx_Undefined;
       ...
       // added by XianHsu
       bool   Single_Injection;     // (true / false) for (single injection / self-regulated jets)
       bool   ClusterMerger_ResetB_VecPot = false;
       double P_inj_Const;          // the constant injection power for single injection
       double T_inj;                // injection time for single injection
       double f_B;                  // fraction of injected energy to be magnetic energy
...
// test whether MHD is defined (xianhsu)
#  ifdef MHD
   if ( MPI_Rank == 0)     Aux_Message( stdout, "   Now using MHD (xianhsu)\n");
#  endif
...
// ********************************************************************************************************************************
   ...
   // added by XianHsu
   ReadPara->Add( "Single_Injection",      &Single_Injection,       false,             Useless_bool,  Useless_bool   );
   ReadPara->Add( "P_inj_Const",           &P_inj_Const,             -1.0,             NoMin_double,  NoMax_double   );
   ReadPara->Add( "T_inj",                 &T_inj,                   -1.0,             NoMin_double,  NoMax_double   );
   ReadPara->Add( "f_B",                   &f_B,                      0.0,             NoMin_double,  NoMax_double   );
...
// convert to code units
   ...
   P_inj_Const       *= 1.0 / UNIT_M / pow(UNIT_L, 2) * pow(UNIT_T, 3);
   T_inj             *= Const_Myr / UNIT_T;
   // Aux_Message( stdout, "  Injection power = %g; Jets duration = %g\n", P_inj_Const, T_inj);
...
// (4) make a note
   if ( MPI_Rank == 0 )
   {
      ...
      Aux_Message( stdout, "=============================================================================\n" );
      Aux_Message( stdout, "  jets power             = %g\n",          P_inj_Const );
      Aux_Message( stdout, "  jets duration          = %g\n",          T_inj );
      Aux_Message( stdout, "  B energy fraction      = %g\n",          f_B );
      Aux_Message( stdout, "=============================================================================\n" );
...
// set the function pointers of various problem-specific routines
   ...
   // if ( MPI_Rank == 0 && Flu_ResetByUser_API_Ptr == Flu_ResetByUser_API_ClusterMerger)    Aux_Message( stdout, "API pointer is repointing...\n");                            

#  ifdef MHD
   Init_Function_BField_User_Ptr  = SetBFieldIC;
   MHD_ResetByUser_BField_Ptr     = MHD_ResetByUser_BField_ClusterMerger;
