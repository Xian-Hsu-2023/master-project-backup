...
titles = ['ref3','ref4','ref5']
file_directory = ['s025_r165_ref3',
                  's026_r165_ref4',
                  's027_r165_ref5']
count = [None]*1000
for i in range(1000):
...
directory = 'wbr165_coolingtime_'+direction+'_range'+str(region)
...
for i in range(num):
    if (i % size == rank):
        ...
        for j in range(len(file_directory)):
            ...
            # sp = ad.cut_region(['obj["gas", "cooling_time"].in_units("Gyr") > 15']) # larger than 25 Gyr is necessary
            p = yt.SlicePlot(ds, direction, 'cooling_time', width=(region, "kpc"))
            # p.annotate_grids()
            p.set_zlim(("gas", "cooling_time"), 1,30)
