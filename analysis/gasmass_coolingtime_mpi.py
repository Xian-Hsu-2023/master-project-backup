...
dir = ['../s020_compare_ref3',
       '../s021_compare_ref4',
       '../s022_compare_ref5',
       '../s023_compare_ref6']
label = ['lv3', 'lv4', 'lv5', 'lv6']
dataname = [None] * 101
for i in range(101):
    dataname[i] = "/Data_" + str(i).zfill(6)
...
for j in range(len(dataname)):
    if j%size==rank:
        ...
        ax.set_xlim(0, 10)
        ax.set_ylim(1e37, 1e40)
        ax.text(0.3, 0.9, 'time = %.2f Myr' %ds.current_time.in_units('Myr'), horizontalalignment='center',verticalalignment='center', transform=ax.transAxes)
        fig.savefig(directory + "/" + str(j).zfill(4) + ".png")
        del fig, ax
        
# mpiexec -n 26 python gasmass_coolingtime_mpi.py