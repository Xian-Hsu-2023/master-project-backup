...
field = "faraday_rotation_" + direction
# directory = args.directory
# titles = [r"$f_B=0.001,\, z=0.1$", r"$f_B=0.001,\, GAMER$", r"$f_B=0.1$"]
titles = ["Regular Test"]
file_directory = ['s011_ref4_nojet']
directory = 'figure/magtest011_epot_'+direction+'_range'+str(region)
...
yt.add_field(("gas", "faraday_rotation_x"), function = _faraday_rotation_x, units="rad/m**2/cm")
yt.add_field(("gas", "faraday_rotation_y"), function = _faraday_rotation_y, units="rad/m**2/cm")
yt.add_field(("gas", "faraday_rotation_z"), function = _faraday_rotation_z, units="rad/m**2/cm")
...
for i in range(num):
    if (i % size == rank):
        ...
        for j in range(len(file_directory)):
            ...
            p = yt.ProjectionPlot(ds, direction, field, width=(region, "kpc"))
            # p.annotate_grids()
            p.set_cmap("bds_highcontrast")
            # p.set_zlim(("gas", field), -1e17, -3e16)
