...
file_directory = ['s004_CompareToFLASH_ref5_0.001',
                  's005_CompareToFLASH_ref4_0.001',
                  's006_CompareToFLASH_ref4_0.1']
directory = 'figure/magtest004_mag_'+direction+'_range'+str(region)
...
for i in range(num):
    if (i % size == rank):
        ...
        for j in range(len(file_directory)):
            ...
            p = yt.SlicePlot(ds, direction, 'magnetic_field_magnitude', width=(region, "kpc"))
            # p.annotate_grids()
            p.set_zlim(("gas", "magnetic_field_magnitude"), 1e-8, 1e-4)
            # p.set_zlim(("gas", "velocity"), 1e-27,2e-25)
            # p.set_zlim(("gas", "density"), 1e-25,2e-24)
            # p.annotate_grids()
            plot = p.plots[("gas", "magnetic_field_magnitude")]
            plot.figure = fig
